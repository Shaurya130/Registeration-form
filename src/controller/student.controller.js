...
const verifyCaptcha = async (req, res) => {
  ...
  try {
    ...
    if (data.success) {
      return res
        .status(200)
        .json({ message: "reCAPTCHA verified successfully" });
    } else {
...
const resendOTP = asyncHandler(async (req, res) => {
  sessionMiddleware(req, res, async () => {
    if (!req.session.userData) {
      throw new ApiError(
        400,
        "User data not found in session. Please start registration again."
      );
    }
    ...
    res
      .status(200)
      .json(new ApiResponse(200, { studentEmail }, "OTP resent successfully."));
  });
});
